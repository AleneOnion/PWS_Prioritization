...
doh<-doh %>%
  select(LOCATION_PWL_ID,LAKE_HISTORY_ID,ID) %>% distinct() %>% 
  rename(PWS.ID=ID)
draft<-merge(draft,doh,by=c('LAKE_HISTORY_ID','LOCATION_PWL_ID'),all.x=TRUE)
save_draft<-draft %>% filter(!is.na(PWS.ID)) %>% distinct()
draft<-draft %>% filter(is.na(PWS.ID)) %>% distinct() %>% select(-PWS.ID)
...
#Now pull SDWIS id
pws2<-readOGR("pws_layer","pws_10_2021") #removed because more accuate list of PWS is from DOH now in our database
wipwl<-readOGR("pws_layer","dec_wipwl_lakes")

#change coords to web mercator for the map
#all of our layers are NAD83 so have to convert to web mercator
wipwl<-sp::spTransform(wipwl, sp::CRS("+proj=longlat +datum=WGS84 +no_defs"))
pws2<-sp::spTransform(pws2, sp::CRS("+proj=longlat +datum=WGS84 +no_defs"))

sf::sf_use_s2(FALSE)
#do left join
pws_merge<-point.in.poly(pws2,wipwl)
...
draft<-merge(draft,save_draft,all=TRUE)
#fill in missing SDWIS ids
draft<-draft %>% 
  mutate(PWS.ID=ifelse(LAKE_HISTORY_ID=='0601WIL0297','NY3800154',PWS.ID),
PWS.ID=ifelse(LAKE_HISTORY_ID=='0905CRA0309','NY4411830',PWS.ID),
PWS.ID=ifelse(LAKE_HISTORY_ID=='1005PIN0398','NY5700124',PWS.ID),
PWS.ID=ifelse(LAKE_HISTORY_ID=='1104ROU0137','NY2800136',PWS.ID),
PWS.ID=ifelse(LAKE_HISTORY_ID=='1202TAN0655A','NY1900033',PWS.ID),
PWS.ID=ifelse(LAKE_HISTORY_ID=='1202TAN0655B','NY1900033',PWS.ID),
PWS.ID=ifelse(LAKE_HISTORY_ID=='1301QUE0184A','NY4317681',PWS.ID),
PWS.ID=ifelse(LAKE_HISTORY_ID=='1301TIO0152','NY3517682',PWS.ID),
PWS.ID=ifelse(LAKE_HISTORY_ID=='1301SUM0193','NY3517676',PWS.ID),
PWS.ID=ifelse(LAKE_HISTORY_ID=='1303ALE0222D','NY3503532',PWS.ID),
PWS.ID=ifelse(LAKE_HISTORY_ID=='1303UPP0223','NY3503526',PWS.ID),
PWS.ID=ifelse(LAKE_HISTORY_ID=='1311THO0274','NY0118342',PWS.ID),
PWS.ID=ifelse(LAKE_HISTORY_ID=='1501STA1011','NY3517675',PWS.ID))
...
```{r, echo=FALSE, results='asis', message=FALSE, warning=FALSE}
draft2<-read.csv("C:/Users/leneo/Dropbox/Alene/Rscripts/PWS_Prioritization/2022_stayCALM_ponded_internal-draft-assessments.csv")
draft2<-draft2 %>% 
  select(seg_id:parameter) %>% 
  distinct() %>% 
  rename(LOCATION_PWL_ID=seg_id) %>% 
  mutate(value=1)
draft2<-merge(pws,draft2,by=c('LOCATION_PWL_ID'),all.x=TRUE)
draft2<-draft2 %>% arrange(LAKE_HISTORY_ID,LOCATION_PWL_ID) %>% 
  distinct(LAKE_HISTORY_ID,.keep_all = TRUE) %>% 
  filter(is.na(use))

sites<-draft2 %>% select(LAKE_HISTORY_ID,PWS)
#Reading in new database
setwd("C:/Users/leneo/Dropbox/Alene/Rscripts/Current")
location<-read.csv("new_database/L_LOCATION.csv",na.strings=c("","NA"), stringsAsFactors=FALSE)
setwd("C:/Users/leneo/Dropbox/Alene/Rscripts/PWS_Prioritization")
