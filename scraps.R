...
rmarkdown::render("2023.02.Lakes.Without.PWLIDs.have.P.numbers.Rmd")
...
ponds<-ponds %>%  filter(!is.na(LAKE_POND_NUMBER)) %>% 
  mutate(LAKE_POND_NUMBER=paste("'P ",LAKE_POND_NUMBER,"' site:",sep="")) %>% 
  select(LAKE_POND_NUMBER) %>% distinct()

#companies <- data.frame(company = c("apple acres llc","abbvie inc","apple inc"))
ponds <- transform(ponds, url = sapply(LAKE_POND_NUMBER,get_first_google_link))
