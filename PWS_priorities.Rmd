...
The following tabs list the unassessed (una.) and unconfirmed (unc.) lakes and streams.

## Una. PWS Lakes

Unassessed public water supplies that are not NYC reservoirs. The data table is sorted by population served when a sidwis id is available.
Please note: this does not include waterbodies without PWLIDs.Those are listed at the end of this document.
...
Unconfirmed public water supplies that are not NYC reservoirs. The data table is sorted by population served when a sidwis id is available.
Please note: this does not include waterbodies without PWLIDs.Those are listed at the end of this document.
...
## PWS lakes without PWLIDs

Map of PWS lakes that don't have PWL IDs
...
Unassessed public water supplies in flowing waters. TO be clear, there multiple intakes on many of these waterbodies.
Please note: this does not include waterbodies without PWLIDs.
...
sites<-sites %>% 
  ...
  mutate(use_assessment="IR3_unassessed") %>% 
  distinct(use_assessment,PWS.ID,PWS.Name,PWS.Type,PopulationServed.Count,WATER_SYSTEM_TYPE,PWL_ID,WIN,NAME,DESCRIPT,PARTITEM,CLASSIFICA,Lat,Long) %>% distinct()
  

#tally results
junk<-sites %>% 
  select(PWL_ID,NAME,use_assessment) %>% distinct() %>% 
  group_by(use_assessment) %>% 
  summarize(n=n()) %>% 
  ungroup()
library(kableExtra)
...
Unconfirmed public water supplies in flowing waters. TO be clear, there multiple intakes on many of these waterbodies.
Please note: this does not include waterbodies without PWLIDs.
...
sites<-sites %>% 
  rename(PWS.ID=WATER_SYSTEMID,
         ...
         PWS.Type=FACILITY_TYPE,
         PopulationServed.Count=PWS_INTAKE_LAYER_POPULATION_SERVED) %>% 
  mutate(use_assessment="unconfirmed") %>% 
  distinct(use_assessment,PWS.ID,PWS.Name,PWS.Type,PopulationServed.Count,WATER_SYSTEM_TYPE,PWL_ID,WIN,NAME,DESCRIPT,PARTITEM,CLASSIFICA,Lat,Long) %>% distinct()
